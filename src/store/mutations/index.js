  ...
  setSortState: (state, { name, method }) => {
    ...
    if (name === 'name') {
      state.students = state.students.sort((a, b) =>
        method === 'down' ? (a.name > b.name ? 1 : -1) : a.name < b.name ? 1 : -1,
      );
    }
    if (name === 'date') {
      state.students = state.students.sort((a, b) =>
        method === 'down' ? (a.date > b.date ? 1 : -1) : a.date < b.date ? 1 : -1,
      );
    }
    if (name === 'russian') {
      state.students = state.students.sort((a, b) =>
    ...
    if (name === 'sum') {
      state.students = state.students.sort((a, b) =>
        method === 'down' ? +a.sum - +b.sum : +b.sum - +a.sum,
      );
    }
    if (name === 'percents') {
      state.students = state.students.sort((a, b) =>
        method === 'down' ? +a.percents - +b.percents : +b.percents - +a.percents,
      );
    }
  },
};
